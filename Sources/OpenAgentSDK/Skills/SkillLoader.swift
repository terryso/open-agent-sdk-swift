...
public enum SkillLoader {
    ...
    static func parseToolDeclarations(
        ...
        // Story 29.5: the tokenizer logic was hoisted into
        // `ToolDeclaration.parse(_:)` so that both this parser and
        // `ToolDeclaration.fromToolNames(_:)` share one tokenizer.
        let declarations = tokens.map { ToolDeclaration.parse($0) }
    ...
    // Story 29.5: the single-token tokenizer (`tokenizeToolDeclaration`), the
    // MCP / base-pattern splitters, and the `ClaudeCodeToolNames` lookup were
    // all hoisted into `ToolDeclaration.parse(_:)` (see
    // `Sources/OpenAgentSDK/Types/ToolDeclaration.swift`). This file now calls
    // that shared parser; the duplicated private implementations are removed.
    ...
    // Story 29.5: the `ClaudeCodeToolNames` private enum was hoisted into
    // `ToolDeclaration` (file-private storage there). This file now reaches the
    // name-recognition logic through `ToolDeclaration.parse(_:)`; the duplicate
    // private enum is removed.
}
