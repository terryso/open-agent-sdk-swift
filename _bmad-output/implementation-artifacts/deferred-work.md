...
## Deferred from: code review of 29-3-direct-skill-package-context (2026-06-14)

- **`extractPromptTextFromRequestBody` does not handle `system` as block-array form** — Helper only processes `system as? String` and `content as? [[String:Any]]` with String `text` values. Anthropic API also allows `system` as a block array and content blocks of non-text types (`tool_use`, `tool_result`, `image`). Not a defect for current usage (Story 29.3 prompt goes into a single text content block), but a future change that splits the prompt across blocks could silently return an empty/incomplete string and cause assertions to fail for the wrong reason. [Tests/OpenAgentSDKTests/MockURLProtocolHelpers.swift:62-77]
- **`nonisolated(unsafe) static var lastRequestBody` race risk** — Both `SkillStreamMockURLProtocol` and `SkillRequestRecordingURLProtocol` declare mutable static state written from the URLSession delegate queue and read/reset from the test thread. `ExecuteSkillTests` has no `setUp`/`tearDown` and relies on manual `reset()`+`defer`. Not flaky under default serial test execution, but would be unsafe under parallel/randomized execution. [Tests/OpenAgentSDKTests/Tools/Advanced/ExecuteSkillStreamTests.swift:513, ExecuteSkillTests.swift:311]
- **Package-context path does not escape `\n` or `---` in supportingFiles path entries** — A supporting file path containing a literal newline or `---` substring would produce a structurally ambiguous prompt. Extremely unlikely from `SkillLoader` (filesystem scan), but no sanitization exists. Would need a separate input-sanitization story. [Sources/OpenAgentSDK/Core/Agent.swift:3257-3263]
- **`trimmedArgs` name is misleading** — Only checks `isEmpty`, does not trim whitespace, so `args = "   "` produces `User request:    ` with literal trailing spaces. Behavior is byte-equal to pre-29.3 (original also used `!args.isEmpty`), so not a regression; the variable name suggests behavior it does not provide. [Sources/OpenAgentSDK/Core/Agent.swift:3243]
- **AC1 absoluteness is not asserted** — Tests check the path substring appears but do not assert the path begins with `/`. Current implementation passes the registered value through unchanged (correct). Could be hardened in Story 29.7. [Tests/OpenAgentSDKTests/Tools/Advanced/ExecuteSkillStreamTests.swift — all baseDir assertions]

## Deferred from: code review of 29-5-shared-filtering-skills-subagents (2026-06-14)

- **Restore race on `options` mutations during concurrent `executeSkillStream`** — `Agent` is `@unchecked Sendable` and `options` is a `var` struct; the streaming path has two restore points (Task body completion + `onTermination`) that can race. Pre-existing pattern for `allowedTools`; Story 29.5 cloned it for `allowedToolDeclarations`. No new category of race introduced. Addressing it requires architectural work on `Agent`'s concurrency model, out of scope for this story. [Sources/OpenAgentSDK/Core/Agent.swift:1383-1395]
- **Stray `)` and `__` inside MCP tool names are edge cases the parser does not gracefully normalize** — `parse("Bash)")` leaves the paren in the base name; MCP tool names containing `__` defeat `isMCPNamespacedName`'s strict 2-part check. Spec-level edge cases (MCP spec does not forbid `__`; `Bash)` form is malformed). Requires parser semantics decisions best made with a broader test corpus. [Sources/OpenAgentSDK/Types/ToolDeclaration.swift:204-241]
- **`patternDeclarations` de-dup by `rawName` and `unmatchedDeclarations` not de-duped** — Duplicate allowed entries (`["Read", "Read"]`) appear twice in `unmatchedDeclarations`; same-pattern declarations with different casing appear as separate `patternDeclarations`. Diagnostics noise only; filtering behavior is correct. [Sources/OpenAgentSDK/Types/ToolDeclaration.swift:395-415]

## Deferred from: code review of 29-6-diagnostics-deferred-subagent-fields (2026-06-14)

- **Whitespace-only string fields (resume/isolation/team_name) treated as deferred** — `collectFieldDiagnostics` uses `!isEmpty` without trimming, so `"  "` (all whitespace) emits a diagnostic with a visually-broken `rawValue`. Matches the spec's own code template (lines 196-217 use the same `.isEmpty` check), so not a deviation — a robustness gap. A future change could add `.trimmingCharacters(in: .whitespaces)` guards. [Sources/OpenAgentSDK/Core/DefaultSubAgentSpawner.swift:206-228]
- **Whitespace-only elements in `skills` array survive comma-join** — `skills.joined(separator: ",")` for `["commit","  ","review"]` yields `"commit,  ,review"`, and for `[""]` yields `""` (indistinguishable from missing). Spec AC7 only mandates "no surrounding whitespace on separator" (satisfied). Per-element filtering is an enhancement. [Sources/OpenAgentSDK/Core/DefaultSubAgentSpawner.swift:230-235]
- **Skill name containing a comma breaks `rawValue` round-trip parse** — `["a,b","c"]` serializes to `"a,b,c"`, ambiguous with `["a","b","c"]`. `rawValue` is documented as the input value stringified, not a parseable serialization. No realistic trigger (skill names do not contain commas). [Sources/OpenAgentSDK/Core/DefaultSubAgentSpawner.swift:233]
- **Control characters in `rawValue` not escaped in rendered tool output** — A `rawValue` containing `\n` would split a diagnostic across lines, technically breaking the "one diagnostic = one line" invariant asserted by `testAgentTool_multipleDiagnostics_renderedInOrder`. Not a security issue (output is post-execution text to the parent LLM). Low likelihood (resume/isolation/team_name come from JSON-decoded `AgentToolInput`). [Sources/OpenAgentSDK/Tools/Advanced/AgentTool.swift:177-181]
- **`result.text` ending in a newline produces a spurious blank line before the diagnostics block** — `output += "\n" + ...` is unconditional. Identical pre-existing behavior for the sibling `[Tools used:]` line; cosmetic regression only. [Sources/OpenAgentSDK/Tools/Advanced/AgentTool.swift:172-181]
