...
_本文档通过逐步发现的方式协作构建。随着我们共同完成每个架构决策，各章节将依次追加。_

## 项目背景分析

_这是一个经过验证的 TypeScript SDK (open-agent-sdk-typescript) 的全新移植项目。架构已从 TypeScript 源码中充分理解，并经过了深入分析。_

### 需求概览

_涵盖 9 个领域的 51 个功能需求 (FR)，6 个类别的 25 个非功能需求 (NFR)。完整详情请参阅 PRD。_

### 技术约束
- **唯一外部依赖：** mcp-swift-sdk (DePasqualeOrg/mcp-swift-sdk) — MCP stdio/SHTTP/SSE
- **自定义 AnthropicClient：** 基于 URLSession，仅支持 POST /v1/messages，不使用社区 Anthropic SDK
- **Swift 5.9+**，macOS 13+，Linux (Ubuntu 20.04+)，不使用 Apple 专属框架
- **模块名称：** `OpenAgentSDK`，仅限 SPM
- **类型系统挑战：** Swift 端使用 Codable 解码，LLM 端使用 `[String: Any]` JSON Schema 字典 — Swift 中没有 Zod 的等价物

- **源码参考：** TypeScript SDK 位于 `/Users/nick/CascadeProjects/open-agent-sdk-typescript/src/`（约 12 个源文件，约 3000-4000 行）

### 横切关注点
1. **并发安全** — 所有可变存储使用 Actor；只读工具使用 TaskGroup 并发执行；串行执行变更操作
2. **错误传播** — 在代理循环中使用带关联值的类型化错误，不中断循环
3. **流式管道** — AsyncStream<SDKMessage> 在并发调度中保持事件顺序
4. **权限执行** — 每次工具执行时的横切拦截器
5. **预算跟踪** — 跨所有 LLM 调用的累计成本，与循环中止集成
6. **JSON Schema 与 Codable 桥接** — 双向类型映射（Swift 端 Codable 解码，LLM 端 JSON Schema）
7. **Shell 钩子执行** — POSIX 进程生成，通过 JSON stdin/stdout 通信
8. **对话压缩** — 自动压缩和微压缩就地修改消息历史

---

## 启动模板评估
_这是一个 Swift SPM 库 — 不是 Web 应用程序。不存在外部启动模板。_

### 选定方案：Swift Package Manager Init
该项目是通过 SPM 分发的 Swift 库。没有 Next.js/Vite/Nuxt 风格的 CLI 启动器。基础是 `swift package init --type library`。

**理由：**
- SPM 是 Swift 生态系统的标准（已通过此方式分发的包只有一个）
...
### AD1: 并发模型 — Swift Actors + 结构化并发

- **决策：** 对所有可变存储使用 `actor`。对工具执行使用 `TaskGroup`（只读，上限 10 个）+ 串行 `for` 循环。
- **理由：** PRD 要求线程安全存储 (FR27, FR48)。Swift actors 提供了编译时安全的隔离，并且不存在锁竞争。`TaskGroup` 以结构化的方式处理 Swift 5.5+ 并发中的任务组。
- **影响：** 所有 6 个存储 + QueryEngine 会话上下文 + MailboxStore
- **TS 参考：** 无参与者 — 模块级变量、函数级并发

- **Swift 模式：** 每个存储是一个 actor；QueryEngine 是一个 actor；工具执行使用带有 maxConcurrency 的 `TaskGroup`

### AD2: 流式模型 — AsyncStream<SDKMessage>
...
- **影响：** QueryEngine.submitMessage() 产生 SDKMessage 值；消费者使用 `for await` 循环
- **TS 参考：** `AsyncGenerator<SDKMessage>` — 使用泛型异步生成器
- **Swift 模式：** `AsyncStream<SDKMessage>`，带有用于模式匹配的 `case let` 语句

### AD3: API 客户端 — 自定义 URLSession AnthropicClient
- **决策：** 在 URLSession 之上构建自定义 `AnthropicClient` actor。仅支持 POST /v1/messages 端点，带流式传输。
...
### AD5: MCP 集成 — mcp-swift-sdk 依赖
- **决策：** 使用 DePasqualeOrg/mcp-swift-sdk 作为 MCP 唯一外部依赖。将 MCP 连接封装在 `MCPClientManager` actor 后。
- **理由：** PRD FR19-FR22 需要 MCP 协议支持。mcp-swift-sdk 提供了经过实战检验的 MCP 客户端+服务器传输（stdio/SSE/HTTP）。如果其不满足需求，则有一个分支并维护的回退方案。
- **影响：** `MCP/` 目录；在代理设置时进行评估（PRD 建议的第 6 阶段）
- **TS 参考：** 使用 `@modelcontextprotocol/sdk` 包进行传输
- **Swift 模式：** `import MCPClient` from mcp-swift-sdk；actor MCPClientManager 按照每个配置管理连接生命周期。MCP 工具通过 `mcp__{serverName}__{toolName}` 命名空间约定进行命名。

### AD6: 会话持久化 — 基于 Actor 的 JSON 文件存储
- **决策：** 使用 `SessionStore` actor 在 `~/.open-agent-sdk/sessions/` 中实现基于 JSON 文件的持久化。
- **理由：** PRD FR23-FR27 要求会话持久化。JSON 文件易于调试，且与 TS SDK 的方法匹配。Actor 隔离可确保多代理并发安全。
...
- **理由：** PRD FR28-FR31 要求 21 个生命周期事件。TypeScript SDK 在 `HookRegistry` 类中使用基于类的方法。Swift 将使用带有静态 case 的枚举（实现穷尽编译时检查）。
- **影响：** `Hooks/` 目录
...
Shell 钩子使用 `Process`（macOS 上的 Foundation，Linux 上使用 `posix_spawn` 或直接使用 `exec` 系列调用）通过 `stdin` 传入 JSON 并从 `stdout` 读取 JSON。
...
### AD10: 错误模型 — 带关联值的类型化错误
- **决策：** 使用带有关联值的嵌套 Swift 枚举。不使用 force-unwrap 或可选值滥用。
- **理由：** NFR15-NFR18 要求可靠的重试/错误处理。Swift 的 typed throws (5.9+) 和带有关联值的枚举支持丰富的错误信息，不会导致意外崩溃。
...
- **理由：** FR7、FR8 要求成本跟踪和预算执行。定价查找表将模型信息与每 token 成本映射。
...
### 关键冲突点（AI 代理可能做出不同选择的领域）
1. **命名约定**
2. **Actor 与非 Actor 边界** — 何时使用 actor 与何时使用 struct/class
3. **错误包装** — 如何将错误从工具传播到循环
4. **Optional 值处理** — nil 与显式默认值
5. **集合类型** — 数组 vs. 集合
6. **JSON 与 Codable 边界** — 何处使用原始 JSON 与 Codable 类型
7. **导入组织** — 按层导入与模块级导入
...
- API 错误：在 `QueryEngine` 中使用重试捕获，在重试耗尽后返回错误结果
...
- 工具属性：通过协议方法如 `isReadOnly`、`isEnabled`
...
### JSON 与 Codable 边界
- **LLM 输入/输出：** 原始 `[String: Any]` 字典（与 API 匹配的 JSON Schema）
- **工具输入：** 通过 `Codable` 从原始 JSON 解码 — 工具闭包接收强类型
- **会话序列化：** `Codable`（JSONEncoder/JSONDecoder）用于磁盘持久化
- **不要将 `Codable` 用于 LLM API 通信** — 使用原始字典类型
...
│       │
│       ├── Stores/
│       │   ├── SessionStore.swift       # Actor: 会话持久化
│       │   ├── TaskStore.swift          # Actor: 任务状态管理
│       │   ├── TeamStore.swift          # Actor: 团队状态管理
│       │   ├── MailboxStore.swift       # Actor: 代理间消息传递
...
| FR3 | `Core/Agent.swift` (`prompt()` 方法) |
| FR4 | `Core/QueryEngine.swift` (submitMessage 循环) |
| FR5 | `Core/QueryEngine.swift` (max_tokens 恢复) |
| FR6 | `Types/AgentTypes.swift`, `Core/QueryEngine.swift` |
| FR7 | `Utils/Tokens.swift`, `Core/QueryEngine.swift` |
| FR8 | `Core/QueryEngine.swift` (循环中的预算检查) |
...
**模式一致性：**
命名约定在整个项目中是一致的。基于 actor 的模式被统一应用。错误处理遵循相同的 typed throws 模式。JSON 与 Codable 边界始终保持在相同的边界（Swift 端使用 `Codable`，LLM 端使用原始字典）。

**结构对齐：**
目录结构通过清晰的模块边界强制执行架构决策。导入规则防止循环依赖。层级组织（Core -> API -> Types）遵循自然的依赖方向。

### 需求覆盖
**所有 51 个 FR 已覆盖：** 每个 FR 都映射到上文记录的特定文件。
**所有 25 个 NFR 已覆盖：**
- 性能 (NFR1-5): AsyncStream + TaskGroup + actor 隔离
- 安全性 (NFR6-10): AnthropicClient 密钥隔离、shell 钩子净化、权限拦截器
- 跨平台 (NFR11-14): 无 Apple 框架、POSIX 路径、SPM CI
- 可靠性 (NFR15-18): Retry.swift、预算优雅关闭、工具错误捕获
- 集成 (NFR19-21): MCPClientManager、AnthropicClient、自定义 baseURL
- API 稳定性 (NFR22-25): 冻结核心 API、演进的 hook/MCP API、semver
...
**未来增强领域：**
- v1.1+: iOS 工具子集审计，PlatformToolSet 协议
- v2.0+: 通过 ModelProvider 协议集成 FoundationModels
...
**实施优先级：**
1. 基础 — `Package.swift`、`Types/`、`API/`、`Utils/`
2. 核心引擎 — `Core/QueryEngine.swift`、`Core/Agent.swift`
3. 工具系统 — `Tools/ToolRegistry.swift`、`Tools/ToolBuilder.swift`、Core 层
4. 高级工具 — Agent、Tasks、Teams
5. 专家工具 — Worktree、Plan、Cron、LSP、Config、Todo
6. MCP 集成 — `Tools/MCP/`
7. 会话与钩子 — `Stores/`、`Hooks/`
8. 完善 — 文档、示例、CI
