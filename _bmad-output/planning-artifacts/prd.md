...
- FR17：Advanced 层工具包括：Agent、SendMessage、TaskCreate/List/Update/Get/Stop/Output、TeamCreate/Delete、NotebookEdit
  ...
  > **术语消歧义（2026-06-14 修订）：** FR17 中的 `TaskCreate/List/Update/Get/Stop/Output` 是**任务管理工具**，由 `TaskStore` actor 支撑，用于代理之间的任务分配与跟踪。
  >
  > Epic 29 引入了一个**独立**的 `Task` 工具，它是 Claude Code `Task(...)` 子代理启动器的兼容入口，作为 `Agent` 工具的 alias（同名 schema、相同执行体）。两者概念不同：
  >
  > | 工具名 | 概念 | 后端 | 引入于 |
  > |---|---|---|---|
  > | `TaskCreate`/`TaskList`/...（FR17） | 任务管理（创建/列出/分配任务） | `Stores/TaskStore.swift` | v1.0 |
  > | `Task`（Epic 29.1） | 子代理启动器 alias | `Tools/Advanced/AgentTool.swift` 共享 factory | Epic 29 |
  >
  > Swift 类型层避免冲突：任务管理工具有独立 input struct（如 `TaskCreateInput`），子代理启动器使用 `SubAgentLauncherInput`（避免与 Swift Concurrency `Task` 冲突，参见 project-context.md 规则 #15）。工具名字符串 `"Task"` 与 `"TaskCreate"` 互不冲突。
